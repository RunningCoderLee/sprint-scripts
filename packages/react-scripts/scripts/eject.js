...
inquirer
  ...
  .then(answer => {
    ...
    if (appPackage.devDependencies) {
      // We used to put sprint-scripts in devDependencies
    ...
    if (fs.existsSync(paths.appTypeDeclarations)) {
      try {
        ...
        // Remove sprint-scripts reference since they're getting a copy of the types in their project
        content =
          content
            // Remove sprint-scripts types
            .replace(
              /^\s*\/\/\/\s*<reference\s+types.+?"sprint-scripts".*\/>.*(?:\n|$)/gm,
              ''
            )
            .trim() + os.EOL;
    ...
    if (ownPath.indexOf(appPath) === 0) {
      try {
        // remove sprint-scripts and sprint-scripts binaries from app node_modules
        Object.keys(ownPackage.bin).forEach(binKey => {
    ...
    if (fs.existsSync(paths.yarnLockFile)) {
      ...
      if (process.platform === 'win32') {
        ...
        // Yarn is diligent about cleaning up after itself, but this causes the sprint-scripts.cmd file
